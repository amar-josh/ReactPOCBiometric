...
interface IBiometricFlowProps {
  ...
  aadhaarVerificationStatus: string | undefined;
...
const BiometricFlow = ({
  ...
  const updateBiometricModalDetails = useCallback(
    (statusKey: string, count: number, message?: string) => {
  ...
  useEffect(() => {
    if (aadhaarVerificationStatus === BIOMETRIC_OPERATIONS.SUCCESS) {
      updateBiometricModalDetails(BIOMETRIC_OPERATIONS.SUCCESS, attemptCount);
    }
  }, [aadhaarVerificationStatus, updateBiometricModalDetails]);

  useEffect(() => {
    if (aadhaarVerificationStatus === BIOMETRIC_OPERATIONS.FAILED) {
      setHasAttemptFailed(true);
      setAttemptCount((prev) => prev - 1);
      updateBiometricModalDetails(
        BIOMETRIC_OPERATIONS.ATTEMPT_FAILED,
        attemptCount - 1,
        validateFingerPrintResponse?.message
      );
    }
  }, [
    aadhaarVerificationStatus,
  ...
  useEffect(() => {
    if (isFingerPrintCaptureSuccess && fingerPrintData) {
      let errCode = null;
      const childrenArray = fingerPrintData?.jsonData?.PidData?.children;
      ...
      if (errCode && errCode !== "0") {
        ...
        // Handle success case
        const base64FingerPrintData = btoa(fingerPrintData?.xmlText);
  ...
  useEffect(() => {
    if (
      isValidateFingerPrintError &&
      (validateFingerPrintError as IValidFingerprintErrorResponse)
        ?.statusCode >= STATUS_CODE.BAD_REQUEST &&
      (validateFingerPrintError as IValidFingerprintErrorResponse)?.statusCode <
    ...
    } else if (isValidateFingerPrintError) {
      setHasAttemptFailed(true);
      updateBiometricModalDetails(
        BIOMETRIC_OPERATIONS.DEFAULT,
        attemptCount,
        validateFingerPrintError?.message
      );
      validateFingerPrintReset();
...
export default memo(BiometricFlow);
